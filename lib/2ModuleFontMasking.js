var backgroundModule2 = (function () {
  let tmp = {};
...
if (document.documentElement.dataset.fbscriptallow !== "true") {
  let scriptModule2Iframe = document.createElement("script");
  scriptModule2Iframe.textContent = `{
  ...
  window.top.document.documentElement.appendChild(scriptModule2Iframe);
  scriptModule2Iframe.remove();
}

window.addEventListener("message", function (e) {
  if (e.data && e.data === "font-fingerprint-defender-alert") {
    backgroundModule2.send("fingerprint", { host: document.location.host });
  }
}, false);
